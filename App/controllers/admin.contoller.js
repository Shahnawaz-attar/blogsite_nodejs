    ...
    result
      .then((data) => {
        if (data != null) {
          res.send({
            status: true,
            url: "/" + req.session.role + "/profile/" + data.id,
            msg: "Update success",
          });
        } else {
          res.send({
...
exports.newslatter_delete = (req, res) => {
  admin_model
    .delete_newslatter(req.params.id)
    .then((post) => {
      if (post != null) {
        res.send({
          status: true,
          url: "/admin/newslatter",
...
exports.save_gallery = (req, res) => {
  let uploaded_files = upload_files.upload.fields([
    ...
    result
      .then((data) => {
        if (data != null) {
          res.send({
            status: true,
            url: "/admin/gallery_list",
            msg: req.body.id ? "Update success" : "Create success",
          });
        } else {
          res.send({
            status: false,
            url: "/admin/gallery_list",
            msg: req.body.id ? "fail to Update" : "fail to Create",
          });
        }
      })
      .catch((err) => {
...
exports.edit_gallery = (req, res) => {
  ...
  Promise.all([edit_gallery, get_images])
    .then((data) => {
      res.render("dashboard/gallery/create-gallery", {
        post: data[0],
        images: data[1],
      });
    })
...
exports.save_files = (req, res) => {
  ...
  uploaded_files(req, res, (err) => {
    ...
    if (extension == "csv") {
      ...
      csv()
        .fromFile(req.files.file[0].path)
        .then((data) => {
          data.forEach((row) => {
            let data = {
              name: row.Name,
